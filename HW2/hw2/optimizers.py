...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        self.velocities = {p: torch.zeros(dp.shape) for p, dp in self.params}
    ...
    def step(self):
        for p, dp in self.params:
            ...
            dp += self.reg * p
            self.velocities[p] = self.momentum * self.velocities[p] - self.learn_rate * dp
            p += self.velocities[p]
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        self.decay = decay
        self.eps = eps
        self.r = {p: torch.zeros(dp.shape) for p, dp in self.params}
    ...
    def step(self):
        for p, dp in self.params:
            ...
            p -= (dp * self.learn_rate / torch.sqrt(self.r[p] + self.eps))
